...
public static class PathManager
{
    ...
    public static void ConvertPathToEdges(Path path, Body body, int subdivisions)
    {
        ...
        {
            for (var i = 1; i < verts.Count; i++) body.AddFixture(new EdgeShape(verts[i], verts[i - 1]));
        }
    }
    ...
    public static void ConvertPathToPolygon(Path path, Body body, float density, int subdivisions)
    {
        ...
        foreach (var item in decomposedVerts) body.AddFixture(new PolygonShape(item, density));
    }
