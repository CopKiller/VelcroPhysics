...
public sealed class TextureConverter
{
    ...
    private void Initialize(uint[] data, int? width, byte? alphaTolerance, float? hullTolerance, bool? holeDetection,
        bool? multipartDetection, bool? pixelOffsetOptimization, Matrix? transform)
    {
    ...
    private bool SearchForOutstandingVertex(Vertices hullArea, out Vector2 outstanding)
    {
        ...
        if (hullArea.Count > 2)
        {
            ...
            for (var i = 1; i < hullAreaLastPoint; i++)
            {
                ...
                if (LineUtils.DistanceBetweenPointAndLineSegment(ref tempVector1, ref tempVector2, ref tempVector3) >=
                    _hullTolerance)
                {
