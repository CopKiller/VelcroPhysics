    ...
    public static bool Cut(World world, Vector2 start, Vector2 end)
    {
        ...
        for (var i = 0; i < fixtures.Count; i++)
        {
            ...
            if (fixtures[i].Body.BodyType != BodyType.Static)
            {
                ...
                {
                    var firstFixture = BodyFactory.CreatePolygon(world, first, fixtures[i].Shape._density,
                ...
                {
                    var secondFixture = BodyFactory.CreatePolygon(world, second, fixtures[i].Shape._density,
                        fixtures[i].Body.Position);
